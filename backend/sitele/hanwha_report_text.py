...
3. **업종 로테이션** 데이터가 있으면 kr_outlook(국내 전망)에서 "최근 어느 업종이 이끌고 어디로 순환매가
   도는가(키맞추기)"를 실제 수치(20일/5일 상위·하위 업종 %)로 규정하라. strategy 에서는 그 로테이션에
   대한 **구체적 대응 액션**(비중 확대/축소·순환매 추종 체크포인트)을 kr_outlook 과 다른 각도로 제시할 것.
4. 9개 섹션(key_issue·bull_case·bear_case·macro_flow·kr_outlook·strategy·news_flow)은 서로 다른 내용이어야 한다.
   같은 문장·문단을 두 섹션에 그대로 복사하지 말 것. 특히 macro_flow(간밤 미국·매크로 흐름)와 kr_outlook(국내 전망),
   그리고 bull_case/bear_case(시나리오)와 strategy(실행 액션)는 반드시 다른 각도로 쓸 것. news_flow 는 비우지 말 것.
5. 지수·종목·매크로·업종수익률은 위 제공 수치만 사용(임의 생성·학습통념 금지).
JSON 형식으로만 출력해 주세요."""
    ...
    for key, value in list(result.items()):
        if isinstance(value, str):
            value = (value.replace("한화생명보험", "한화손해보험").replace("한화생명", "한화손해보험")
                     .replace("모멤타임", "모멘텀").replace("모멘타임", "모멘텀"))
