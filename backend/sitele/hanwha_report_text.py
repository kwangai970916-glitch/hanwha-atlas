...
3. **업종 로테이션** 데이터가 있으면 sector_strategy 블록에서 "최근 어느 업종이 이끌고 어디로 순환매가
   도는가(키맞추기)"를 실제 수치(20일/5일 상위·하위 업종 %)로 규정하고, 그에 따른 실행 가능한 대응 전략을 제시할 것.
4. 네 블록(global_kr·catalysts·sector_strategy·checkpoint)은 각자 고유 역할을 지킬 것.
   global_kr=간밤 미국·매크로, sector_strategy=국내 심리·밸류·로테이션 대응 — 같은 문단을 다른 블록에 복사 금지.
